...
fp_relu_model = create_custom_neuron_class(
    'fp_relu',
...
class FPReluNeurons(Neurons):
    pipelined = True

    def __init__(self, K=10, alpha=25):
        super(FPReluNeurons, self).__init__()
        self.K = K
        self.alpha = alpha
    ...
    def compile(self, mlg_model, layer):
        ...
        for u in layer.upstream_synapses:
            ...
            upstream_relu_input = isinstance(nrn, FPReluInputNeurons)
            if upstream_relu or upstream_relu_input:
                # Check K parameters match
                if nrn.K != self.K:
                    raise ValueError("K parameters of FP ReLU neurons must "
            ...
            # Otherwise, give error
                ...
                raise ValueError("FP neurons can only be connected "
                                 "to other FP neurons")
        ...
        super(FPReluNeurons, self).compile(mlg_model, layer, model,
                                           params, vars, {})

    def set_threshold(self, threshold):
        raise NotImplementedError('FP neurons do not have '
